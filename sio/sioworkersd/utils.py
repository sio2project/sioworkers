...
def get_required_ram_for_job(env):
    ...
    if job_type.endswith('exec'):
        required_ram = env.get('exec_mem_limit', DEFAULT_RAM_REQUIREMENTS['exec'])
        # We need to make sure that we have enough ram for a checker as well.
        if env.get('check_output'):
            required_ram = max(
                required_ram,
                env.get('checker_mem_limit', DEFAULT_RAM_REQUIREMENTS['checker']),
            )
    else:
        required_ram = env.get(
            job_type + '_mem_limit',
            DEFAULT_RAM_REQUIREMENTS.get(job_type, DEFAULT_RAM_REQUIREMENTS['default']),
        )
